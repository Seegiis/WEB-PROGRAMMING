...
function addWeekDays(tblBody2) {
	...
	var textVals3 = ["Mo", "Tu", "We", "Th", "Fr", "Sa", "Su"];
	for (var i = 0; i < 7; i++) {
...
function addAllDates(tblBody3, date, callback, id) {
	...
	if (startDayOfMonth !== 0) {
		...
		for (var i = 0; i < startDayOfMonth; i++) {
			...
			var cell4 = createPreviousMonthCell(id, date, cell4.textContent, callback);
			cell4.appendChild(val4);
			cell4.setAttribute("class", "gray");
			console.log(date.getMonth()+ " sariin " + cell4.textContent);
	...
	if (day !== 0) {
		for (var i = day; i < 7; i++) {
			...
			cell=createNextMonthCell(id, date,cell.textContent, callback);
			cell.appendChild(val);
			cell.setAttribute("class", "gray");
			console.log(date.getMonth()+ 2 + " sariin " + cell.textContent);
			row.appendChild(cell);
		}
		tblBody3.appendChild(row);
	}
...
function clickDate(id, date, cell, callback) {
	date.day = cell.textContent;
	date.year = date.getFullYear();
	date.month = date.getMonth() + 1;
	callback.call(this, id, date);

}
function createNextMonthCell(id, date, val, callback) {
	var cell = document.createElement("td");
	cell.addEventListener("click", function() {
		clickNextMonthsDate(id, date, cell, callback);
	});
	var val = document.createTextNode(val);
	cell.appendChild(val);
	cell.setAttribute("class", "black");
...
function clickNextMonthsDate(id, date, cell, callback) {
	date.day = cell.textContent;
	date.year = date.getFullYear();
	date.month = date.getMonth() + 2;
	callback.call(this, id, date);

}
function createPreviousMonthCell(id, date, val, callback) {
	var cell = document.createElement("td");
	cell.addEventListener("click", function() {
		clickPreviousMonthsDate(id, date, cell, callback);
	});
	var val = document.createTextNode(val);
	cell.appendChild(val);
	cell.setAttribute("class", "black");
...
function clickPreviousMonthsDate(id, date, cell, callback) {
	date.day = cell.textContent;
	date.year = date.getFullYear();
